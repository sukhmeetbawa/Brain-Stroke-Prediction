...
def ProcessUserinfo(userinfo):
    ...
    work = {'Private': 1, 'Self-employed': 2,
    ...
    smoking = {'smokes': 1, 'never smoked': 2,
    ...
    def ensemble(X):
        ...
        for i in range(len(predictionDT)):
            ensemble_prediction[i] = predictionDT[i] + \
                predictionKN[i] + predictionNB[i]+predictionRFC[i]
    ...
    work = {'Private': 1, 'Self-employed': 2,
            'Govt-job': 3, 'Children': 4, 'Never worked': 5}
